...
import random
from dataset.ImageCoder import ImageCoder
...
class TFRecords(object):
    ...
    def convert_to_example(self, images, labels, name):
        """Build an Example proto for an example.
        Args:
            images: 4D tensor, (NUM_IMAGES, 3, width, height)
            labels: 1D tensor, (NUM_IMAGES,)integer,
            name: string, name of the dataset subset
        Returns:
            TFRecords file
        """
        ...
        filename = os.path.join(cfg.TRAIN.data_dir, name + '.tfrecords')
        print('Writing', filename)
        writer = tf.python_io.TFRecordWriter(filename)
        for index in range(num_examples):
            image_raw = images[index].tostring()
            example = tf.train.Example(features=tf.train.Features(feature={
                'image/height': self._int64_feature(rows),
                'image/width': self._int64_feature(cols),
                'image/depth': self._int64_feature(depth),
                'image/class/label': self._int64_feature(int(labels[index])),
                'image/image_raw': self._bytes_feature(image_raw)}))
